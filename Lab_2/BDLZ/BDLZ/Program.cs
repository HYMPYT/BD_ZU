...
namespace BDLZ
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string connectionString = "Host=localhost;Username=postgres;Password=p;Database=bdlz2";
            ...
            do
            {
                ...
                switch (table)
                {
                    case 1:
                        ...
                        controller = new FacultyController(connectionString);
                        break;
                    case 2:
                        action = SecondMenu("Group");
                        controller = new GroupController(connectionString);
                        break;
                    case 3:
                        action = SecondMenu("Lectures");
                        controller = new LecturesPlanController(connectionString);
                        break;
                    case 4:
                        action = SecondMenu("Student");
                        controller = new StudentController(connectionString);
                        break;
                    case 5:
                        action = SecondMenu("Subject");
                        controller = new SubjectController(connectionString);
                        break;
                    case 6:
                        action = SecondMenu("Teacher");
                        controller = new TeacherController(connectionString);
                        break;
                }
